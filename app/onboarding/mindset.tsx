...
import { Lock, Leaf } from 'lucide-react-native';
...
export default function MindsetScreen() {
  ...
  const animateToNextStep = () => {
    ...
    setTimeout(() => {
      router.push('/onboarding/extraversion'); // Next step is extraversion slider
    }, 400);
  };

  const handleCardChoice = (choice: 'fixed' | 'growth') => {
    updateOnboardingData({ mindset: choice });
              ...
              <View style={styles.stepContainer}>
                <View style={styles.interactionSection}>
                  ...
                  <View style={styles.cardsContainer}>
                    <TouchableOpacity
                      style={styles.card}
                      onPress={() => handleCardChoice('fixed')}
                    ...
                    <TouchableOpacity
                      style={styles.card}
                      onPress={() => handleCardChoice('growth')}
                      onPressIn={handleInteractionStart}
                      onPressOut={handleInteractionEnd}
                      activeOpacity={0.8}
                      disabled={interactionState === 'selected' || interactionState === 'transitioning'}
